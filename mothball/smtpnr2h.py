...
def place_constraints_2d(comps, fab_dims, wire_lengths, pack=True):
    ...
    for comp in comps:
        for adj in comp.inputs:
            ...
            constraints.append(z3.Or(_get_x(comp.pos) == z3.LShR(_get_x(adj.pos), shx),_get_x(comp.pos) == _get_x(adj.pos) << shx ))
            constraints.append(z3.Or(_get_y(comp.pos) == z3.LShR(_get_y(adj.pos), shy), _get_y(comp.pos) == _get_y(adj.pos) << shy))
...
def place_constraints_opt(comps, fab_dims, distinct_flag=True):
  ...
  #form the manhattan distance
    ...
    for comp in comps:
        for adj in comp.inputs:
            manhattan_dist.append(abs_diff(comp.pos[0], adj.pos[0], fab_dims) + abs_diff(comp.pos[1], adj.pos[1], fab_dims))
...
def run_test(adj, fab_dims, wire_lengths={}, debug_prints=True, 
    ...
    if wire_lengths: #use provided wire lengths
        ...
        constraints, shiftvars = constraints_gen(comps, fab_dims, wire_lengths)
        s = z3.Optimize()
        s.add(constraints)
        frows,fcols = fab_dims
        manhattan_dist = 0
        for sh in shiftvars:
            manhattan_dist += z3.ZeroExt(8,sh)
        s.minimize(manhattan_dist)
        if s.check() != z3.sat:
            if debug_prints:
                print('test is unsat')
            return s
        ...
        if debug_prints:
            print('test is sat')
            #print('L1 Norm = ', s.model().evaluate(manhattan_dist))
